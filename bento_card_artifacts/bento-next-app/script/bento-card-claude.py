...
# Initialize the Anthropic client with AiHubMix base URL
client = anthropic.Anthropic(
    api_key=api_key,  # 🔑 从 .env.local 读取
    base_url="https://aihubmix.com"
)

# Example content similar to the Gemini example
...
# Create the message with system and user content
...
for content_block in response.content:
    if content_block.type == 'text':
        ...
        # Try to extract HTML code block from the text response
        html_match = re.search(r'```html\s*(<!DOCTYPE html>[\s\S]*?)<\/html>[\s\S]*?```', text)
        if html_match:
